...
public class EmpresaTelecomunicaciones {
	...
	// Buscar un cliente con su nombre y actualizar sus datos de nombre y
	// direccion
	public boolean actualizarCliente(String nombreOriginal, String nuevoNombre,
			String nuevaDireccion) throws UbicacionInvalidaException,
	...
	public void agregarRepresentante(String nombreCompleto, String numId)
			throws NombreInvalidoException, CarnetIdentidadInvalidoException,
			DuplicadosException {
	...
	public void agregarEntidadNoEstatal(String nombreEntidad, String direccion,
			Representante representante) throws NombreInvalidoException,
			UbicacionInvalidaException, DuplicadosException {
			...
			if (c instanceof EntidadNoEstatal) {
				...
				if (ene.getDireccion().equalsIgnoreCase(direccion))
					throw new DuplicadosException(
							"Esa persona ya existe en nuestro sistema");
			}
		}

		Cliente c1 = new EntidadNoEstatal(nombreEntidad, direccion,
				representante);
		clientes.add(c1);
	}

	// Agregar Persona Natural
	public void agregarPersonaNatural(String nombre, String direccion,
			String municipio, String provincia, String numId)
		...
		Cliente c1 = new PersonaNatural(nombre, direccion, municipio,
				provincia, numId);
	...
	public void agregarPersonaJuridica(String nombreEmpresaString,
		...
		Cliente c1 = new PersonaJuridica(nombreEmpresaString, direccion,
				municipio, provincia, organismo, representante);
	...
	public void crearCuentaNauta(Cliente titular, String nick) {
		...
		if (!tieneTelefono)
			throw new IllegalArgumentException(
					"Para contratar el nauta debe tener teléfono fijo");

		if (titular instanceof PersonaJuridica)
			throw new IllegalArgumentException(
					"Este tipo de cliente no puede tener CuentaNauta");

		if (comprobarNauta(titular))
			throw new IllegalArgumentException(
					"El cliente ya tiene una CuentaNauta");
	...
	// Metodo para validar si persona natural ya tiene cuenta nauta o si es
	// persona juridica
	...
	public void asignarTelefonoFijo(Cliente titular, String numero) {
		...
		if (disponible == null)
			throw new IllegalArgumentException(
					"No hay teléfono fijo disponible con el número especificado.");

		if (comprobarFijoPersonaNatural(titular))
			throw new IllegalArgumentException(
					"Persona Natural no puede tener más de 1 teléfono fijo.");
	...
	public boolean eliminarTelefonoFIjo(String numero) {
		...
		if (fijoAEliminar != null) {
			...
			ArrayList<Servicio> serviciosCliente = new ArrayList<>(
					titular.getServicios());
			for (Servicio c : serviciosCliente) {
				if (c instanceof TelefonoFijo) {
					if (((TelefonoFijo) c).getNumero().equals(
							fijoAEliminar.getNumero())) {
			...
			for (Servicio s : titular.getServicios()) {
				if (s instanceof TelefonoFijo) {
					System.out.println(((TelefonoFijo) s).getNumero().equals(
							fijoAEliminar.getNumero()));
	...
	public void asignarTelefonoMovil(Cliente titular) {
		...
		if (disponible == null)
			throw new IllegalArgumentException(
					"No hay teléfono móvil disponible");

		if (comprobarMovil(titular))
			throw new IllegalArgumentException(
					"Persona Natural no puede tener más de 2 Teléfonos Móviles");
		...
		if (!servicios.isEmpty()) { // Se comprueba que hay al menos un servicio
			for (Servicio s : servicios) {
				if (s instanceof TelefonoMovil) {
					...
					// Buscamos la cantidad de llamadas que superan los X min
					// del telefono
	...
	// Buscar los clientes que tengan al menoos 30% (4 ) meses de mas de 1000
	// cup de montoTotal en sus Cuentas Nautas
		...
		if (servicios != null) {
			...
			for (Servicio s : servicios) {
				if (s instanceof CuentaNauta) {
					...
					HashMap<String, Double> mesesGasto = cuentaActual
							.calcularKbGastadosMeses();
					HashMap<String, Double> mesesMayores = cuentaActual
							.buscarMesesMayores(mesesGasto);

					double gastoKbMesesMayores = Collections.max(mesesMayores
							.values()); // Obtener el primer
					// valor(Todos iguales)
					...
					if (gastoKbMesesMayores > mayor) {
						...
						for (String m : mesesMayores.keySet()) {
							if (!(mesesMayorGastoCuentas.contains(m))) // /
																		// Evitamos
																		// que
																		// se
																		// guarden
	...
	public ArrayList<Map.Entry<String, Integer>> menorCantCuentasNauta() {
		...
		String[] provincias = { "Pinar del Rio", "Artemisa", "La Habana",
		...
		if (servicios != null) {
			for (Servicio s : servicios) {
				if (s instanceof CuentaNauta) {
					...
					if (cuentaNauta.getTitular() != null
							&& cuentaNauta.getTitular() instanceof PersonaNatural)
						personasNaturales.add((PersonaNatural) cuentaNauta
								.getTitular());
				}
			}
		}
		...
		for (PersonaNatural p : personasNaturales) {
			...
			if (provinciasConCuenta.containsKey(provinciaCliente)) {
				provinciasConCuenta.put(provinciaCliente,
						provinciasConCuenta.get(provinciaCliente) + 1);
			}
		}
	...
	// Clientes con un Consumo mayor a 500 pesos en al menos 3 Llamadas de Larga
	// Distancia
	// Se puede mejorar el metodo devolviendo un hashmap con los nombres y el
	// monto
		...
		for (Servicio s : servicios) {
			if (s instanceof TelefonoFijo) {
				...
				if (!clientesEvaluados.contains(titular)) {
					...
					// Buscamos nuevamente en todos los servicios si ese cliente
					// tiene mas de 1 telefono fijo
					for (Servicio otro : servicios) {
						if (otro instanceof TelefonoFijo) {
							...
							if (otroTf.getTitular().equals(titular)) {
								for (LlamadaLargaDistancia llamada : otroTf
										.getLlamadasLargas()) {
	...
	// Buscar los representantes que no tienen clientes a representar
	...
	// Mï¿½todo para asignar representante a un cliente (Persona Jurï¿½dica o
	// Entidad No Estatal)
	public void asignarRepresentanteACliente(Cliente cliente,
		...
		if (cliente != null && representante != null) {
			...
			if (cliente instanceof PersonaJuridica) {
				...
				if (pj.getRepresentantePersonaJuridica() != null) {
					pj.getRepresentantePersonaJuridica()
							.setClienteRepresentado(null);
				}
		...
		if (serviciosDisponibles != null) {
			...
			for (int i = 0; i < serviciosDisponibles.size(); i++) {
				...
				if (serviciosDisponibles.get(i) instanceof Telefono) {
					...
					if (((Telefono) serviciosDisponibles.get(i)).getNumero()
							.equals(numero)) {
	...
	// Clientes con todos los servicios contratados(logica y escalabilidad
	// mejorada) :)
