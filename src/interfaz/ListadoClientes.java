...
public class ListadoClientes extends JDialog {
	...
	private JLabel lblRepresentanteJuridicaCreate; // Para Persona Jurídica
													// (creación)
	private JLabel lblRepresentanteEntidadCreate; // Para Entidad No Estatal
													// (creación)
	...
	private JLabel lblNombreEdit, lblDireccionEdit, lblMunicipioEdit,
			lblProvinciaEdit, lblNumIdEdit, lblOrganismoEdit;
	private JLabel lblNombreCreate, lblDireccionCreate, lblMunicipioCreate,
			lblProvinciaCreate, lblNumIdCreate, lblOrganismoCreate;
	...
	private static final Map<String, String[]> PROVINCIAS_MUNICIPIOS = new LinkedHashMap<String, String[]>() {
		{
			put("Pinar del Río", new String[] { "Consolación del Sur", "Guane",
					"La Palma", "Los Palacios", "Mantua",
					"Minas de Matahambre", "Pinar del Río",
					"San Juan y Martínez", "San Luis", "Sandino", "Viñales" });
			put("Artemisa", new String[] { "Alquízar", "Artemisa",
					"Bahía Honda", "Bauta", "Caimito", "Candelaria",
					"Guanajay", "Güira de Melena", "Mariel",
					"San Antonio de los Baños", "San Cristóbal" });
			put("La Habana", new String[] { "Arroyo Naranjo", "Boyeros",
					"Centro Habana", "Cerro", "Cotorro", "Diez de Octubre",
					"Guanabacoa", "La Habana del Este", "La Habana Vieja",
					"La Lisa", "Marianao", "Playa", "Plaza de la Revolución",
					"Regla", "San Miguel del Padrón" });
			put("Mayabeque", new String[] { "Batabanó", "Bejucal", "Güines",
					"Jaruco", "Madruga", "Melena del Sur", "Nueva Paz",
					"Quivicán", "San José de las Lajas", "San Nicolás",
					"Santa Cruz del Norte" });
			put("Matanzas",
					new String[] { "Calimete", "Cárdenas",
							"Ciudad de Matanzas", "Colón", "Jagüey Grande",
							"Jovellanos", "Limonar", "Los Arabos", "Martí",
							"Pedro Betancourt", "Perico", "Unión de Reyes",
							"Varadero" });
			put("Cienfuegos", new String[] { "Abreus", "Aguada de Pasajeros",
					"Cienfuegos", "Cruces", "Cumanayagua", "Lajas", "Palmira",
					"Rodas" });

			put("Villa Clara", new String[] { "Caibarién", "Camajuaní",
			...
			put("Sancti Spíritus", new String[] { "Cabaiguán", "Fomento",
					"Jatibonico", "La Sierpe", "Sancti Spíritus", "Taguasco",
					"Trinidad", "Yaguajay" });
			put("Ciego de Ávila", new String[] { "Baraguá", "Bolivia",
					"Chambas", "Ciego de Ávila",

					"Ciro Redondo", "Florencia", "Majagua", "Morón",
					"Primero de Enero", "Venezuela" });

			put("Camagüey", new String[] { "Camagüey", "Carlos M. de Céspedes",
					"Esmeralda", "Florida", "Guáimaro", "Jimaguayú", "Minas",
					"Najasa", "Nuevitas", "Santa Cruz del Sur", "Sibanicú",
					"Sierra de Cubitas", "Vertientes" });
			put("Las Tunas", new String[] { "Amancio", "Colombia",
					"Jesús Menéndez", "Jobabo", "Las Tunas", "Majibacoa",
					"Manatí", "Puerto Padre" });
			put("Holguín", new String[] { "Antilla", "Báguanos", "Banes",
					"Cacocum", "Calixto García", "Cueto", "Frank País",
					"Gibara",

					"Holguín", "Mayarí", "Moa", "Rafael Freyre",
					"Sagua de Tánamo", "Urbano Noris" });

			put("Granma", new String[] { "Bartolomé Masó", "Bayamo",
					"Buey Arriba", "Campechuela", "Cauto Cristo", "Guisa",
					"Jiguaní", "Manzanillo", "Media Luna", "Niquero", "Pilón",
					"Río Cauto", "Yara" });
			put("Santiago de Cuba", new String[] { "Contramaestre", "Guamá",
					"Mella", "Palma Soriano", "San Luis", "Santiago de Cuba",
					"Segundo Frente", "Songo-La Maya", "Tercer Frente" });
			put("Guantánamo", new String[] { "Baracoa", "Caimanera",
					"El Salvador", "Guantánamo", "Imías", "Maisí",
					"Manuel Tames", "Niceto Pérez", "San Antonio del Sur",
					"Yateras" });
	...
	private ListadoClientes(Principal principal) {
		...
		setModal(true);
		setResizable(false); // ← Esto evita que la ventana se pueda
								// redimensionar
	...
	// Añadir nuevo método para filtrar la tabla (similar al de
	// ListadoRepresentante)
	private void filtrarTabla() {
		...
		try {
			...
			ArrayList<Cliente> todosClientes = EmpresaTelecomunicaciones
					.getInstancia().getClientes();

			for (Cliente cliente : todosClientes) {
				boolean coincideNombre = cliente.getNombre().toLowerCase()
	...
	private void initPanelCreacion() {
		...
		cbTipoCliente.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String tipoSeleccionado = (String) cbTipoCliente
						.getSelectedItem();
		...
		cbProvinciaCreate.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String provinciaSeleccionada = (String) cbProvinciaCreate
						.getSelectedItem();
		...
		btnSeleccionarRepresentanteCreate = new JButton(
				"Seleccionar Representante");
		btnSeleccionarRepresentanteCreate.setForeground(new Color(0, 0, 153));
		btnSeleccionarRepresentanteCreate
				.setBackground(new Color(255, 255, 255));
		btnSeleccionarRepresentanteCreate.setFont(new Font("Serif", Font.PLAIN,
				18));
		...
		lblRepresentanteJuridicaCreate
	...
	// Metodo para le eleccion de un Representante en la tabla de servicio al
	// crear a un cliente

	private void seleccionarRepresentanteCreacion() {
		if (EmpresaTelecomunicaciones.getInstancia()
				.buscarRepresentantesLibres().size() > 0) {
			Representante rep = ListadoSeleccionRepresentante
					.abrirYSeleccionar();
			if (rep != null) {
				...
				if (lblRepresentanteJuridicaCreate != null) {
					lblRepresentanteJuridicaCreate.setText("Representante: "
							+ rep.getNombreCompleto());
				}
				if (lblRepresentanteEntidadCreate != null) {
					lblRepresentanteEntidadCreate.setText("Representante: "
							+ rep.getNombreCompleto());
				}
			}
		} else {
			...
			JOptionPane.showMessageDialog(this,
					"No hay representantes disponibles", "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}

	// Metodo para le eleccion de un Servicio en la tabla de servicio al crear a
	// un cliente
	private void seleccionarServicio() {
		ArrayList<Servicio> serviciosDisponibles = EmpresaTelecomunicaciones
				.getInstancia().getServiciosDisponibles();
		...
		if (serviciosDisponibles != null && !serviciosDisponibles.isEmpty()) {
			...
			if (servicio != null) { // Verificar que el servicio no sea null
				...
				UIManager.put("OptionPane.messageFont", new Font("Serif",
						Font.PLAIN, 18));
				JOptionPane.showMessageDialog(this,
						"Servicio asignado correctamente", "Éxito",
	...
	// Metodo para cargar los municipios en el ComboBox en dependencia de la
	// provincia actual
	...
	// Metodo para mostrar solamente los campos necesarios para cada tipo de
	// cliente
	...
	// Metodo para crear un nuevo cliente en dependencia del cliente que se este
	// intentnando crear
	private void crearCliente() {
		...
		try {
			...
			if ((tipoCliente.equals("Persona Jurídica") || tipoCliente
					.equals("Entidad No Estatal"))
					&& representanteSeleccionado == null) {
			...
			if (errores.length() > 0) {
				if (representanteSeleccionado == null
						&& servicioSeleccionado == null
						&& !(tipoCliente.equals("Persona Natural"))) {
					errores.insert(22, " y ");
				}

				UIManager.put("OptionPane.messageFont", new Font("Serif",
						Font.PLAIN, 18));
				JOptionPane.showMessageDialog(this, "Error: Debe seleccionar "
						+ errores.toString(), "Error",
						JOptionPane.ERROR_MESSAGE);
			}
			...
			if (tipoCliente.equals("Persona Natural")) {
				...
				if (creado) {
					...
					int longitudListado = EmpresaTelecomunicaciones
							.getInstancia().getClientes().size();
					nuevoCliente = EmpresaTelecomunicaciones.getInstancia()
							.getClientes().get(longitudListado - 1);
			...
			// CREAR UNA PERSONA JURIDICA
				...
				creado = crearPersonaJuridica(servicioSeleccionado,
				...
				if (creado) {
					...
					if (representanteSeleccionado != null) {
						EmpresaTelecomunicaciones.getInstancia()
								.asignarRepresentanteACliente(nuevoCliente,
										representanteSeleccionado);
					}
				}
			} else if (tipoCliente.equals("Entidad No Estatal")) {

				creado = crearEntidadNoEstatal(servicioSeleccionado,
						representanteSeleccionado);
			...
			if (nuevoCliente != null) {
				...
				EmpresaTelecomunicaciones.getInstancia().getServicios()
						.add(servicioSeleccionado);
				EmpresaTelecomunicaciones.getInstancia()
						.getServiciosDisponibles().remove(servicioSeleccionado);
				...
				UIManager.put("OptionPane.buttonFont", new Font("Serif",
						Font.PLAIN, 16));
				JOptionPane.showMessageDialog(this,
						"Cliente creado exitosamente", "Éxito",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				throw new Exception("No se pudo crear el cliente");
			}
		} catch (Exception e) {
			// En caso de error, liberar cualquier recurso asignado en la BD de
			// la empresa o en el servicio seleccionado
			if (nuevoCliente != null) {
				EmpresaTelecomunicaciones.getInstancia().getClientes()
						.remove(nuevoCliente);
			}
			if (servicioSeleccionado != null
					&& servicioSeleccionado.getTitular() != null) {
	...
	private void initPanelEdicion() {
		...
		cbProvinciaEdit.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String provinciaSeleccionada = (String) cbProvinciaEdit
						.getSelectedItem();
		...
		lblRepresentanteSeleccionadoEdit.setFont(new Font("Serif", Font.PLAIN,
				16));
		...
		// Botón para seleccionar representante
		JButton btnSeleccionarRepresentanteEdit1 = new JButton(
				"Seleccionar Representante");
		btnSeleccionarRepresentanteEdit1.setForeground(new Color(0, 0, 153));
		btnSeleccionarRepresentanteEdit1
				.setBackground(new Color(255, 255, 255));
		btnSeleccionarRepresentanteEdit1.setFont(new Font("Serif", Font.PLAIN,
				18));
		...
		lblRepresentanteSeleccionadoEdit1.setFont(new Font("Serif", Font.PLAIN,
				16));
		lblRepresentanteSeleccionadoEdit1.setBounds(35, 90, 280, 30); // Ajusta
																		// posición
																		// según
																		// necesites
		...
		if (EmpresaTelecomunicaciones.getInstancia()
			...
			if (rep != null) {
				...
				if (lblRepresentanteSeleccionadoEdit != null) {
					lblRepresentanteSeleccionadoEdit.setText("Representante: "
							+ rep.getNombreCompleto());
				}
				if (lblRepresentanteSeleccionadoEdit1 != null) {
					lblRepresentanteSeleccionadoEdit1.setText("Representante: "
							+ rep.getNombreCompleto());
				}
			}
		} else {
	...
	private void mostrarCamposSegunTipoEdicion(Cliente cliente) {
		...
		if (cliente instanceof PersonaNatural) {
			...
			if (pj.getRepresentante() != null) {
				lblRepresentanteSeleccionadoEdit.setText("Representante: "
						+ pj.getRepresentante().getNombreCompleto());
				representanteSeleccionado = pj.getRepresentante();
			} else {
				lblRepresentanteSeleccionadoEdit
						.setText("Representante: Ninguno");
				representanteSeleccionado = null;
			}
			...
			if (ene.getRepresentanteEntidad() != null) {
				lblRepresentanteSeleccionadoEdit1.setText("Representante: "
						+ ene.getRepresentanteEntidad().getNombreCompleto());
				representanteSeleccionado = ene.getRepresentanteEntidad();
			} else {
				lblRepresentanteSeleccionadoEdit1
						.setText("Representante: Ninguno");
				representanteSeleccionado = null;
			}
		}
		...
		try {
			...
			if (clienteActual == null) {
				UIManager.put("OptionPane.messageFont", new Font("Serif",
						Font.PLAIN, 18));
				JOptionPane.showMessageDialog(this,
						"El cliente ha sido eliminado", "Error",
						JOptionPane.ERROR_MESSAGE);
				cerrarPanelEdicion();
				return;
			}
			...
			try {
				...
				if (clienteActual instanceof PersonaJuridica) {
					this.validarNombreEdicionRobustaJ(txtNombreEdit.getText(),
							clienteActual);
				}
				if (clienteActual instanceof EntidadNoEstatal) {
					this.validarNombreEdicionRobustaE(txtNombreEdit.getText(),
							clienteActual);
			...
			if (clienteSeleccionado instanceof PersonaNatural) {
				...
				try {
					this.validarNumIdEdicion(txtNumIdEdit.getText(),
							clienteActual);
					// PRIMERO VERIFICAR SI EL NÚMERO YA EXISTE
					for (Cliente c : EmpresaTelecomunicaciones.getInstancia()
							.getClientes()) {
						if (c instanceof PersonaNatural) {
							PersonaNatural pn = (PersonaNatural) c;
							if (pn != clienteActual
									&& pn.getNumId().equals(
											txtNumIdEdit.getText())) {

								throw new DuplicadosException(
										"Ya existe una persona con este número de identidad");
							}
						}
					}
					for (Representante r : EmpresaTelecomunicaciones
							.getInstancia().getRepresentantes()) {
						System.out.println(r.getNumId() + " VS  "
								+ txtNumIdEdit.getText());
			...
			if (errores.length() > 0) {
				...
				// Guardar el representante anterior para limpieza posterior si
				// es necesario
				Representante representanteAnterior = null;
				if (clienteSeleccionado instanceof PersonaJuridica) {
					representanteAnterior = ((PersonaJuridica) clienteSeleccionado)
							.getRepresentantePersonaJuridica();
				} else if (clienteSeleccionado instanceof EntidadNoEstatal) {
					representanteAnterior = ((EntidadNoEstatal) clienteSeleccionado)
							.getRepresentanteEntidad();
				}
				...
				if (clienteSeleccionado instanceof PersonaNatural) {
					...
					if (representanteSeleccionado != null) {
						// Desasignar primero el representante anterior si es
						// diferente al nuevo
						if (representanteAnterior != null
								&& !representanteAnterior
										.equals(representanteSeleccionado)) {
							EmpresaTelecomunicaciones.getInstancia()
									.desasignarRepresentanteDeCliente(
											clienteSeleccionado);
						}
						// Asignar el nuevo representante
						EmpresaTelecomunicaciones.getInstancia()
								.asignarRepresentanteACliente(
										clienteSeleccionado,
										representanteSeleccionado);
					} else {
						// Si no hay representante seleccionado, desasignar el
						// existente
						EmpresaTelecomunicaciones.getInstancia()
								.desasignarRepresentanteDeCliente(
										clienteSeleccionado);
	...
	private void validarNombreEdicion(String nombre)
	...
	private void validarNombreEdicionRobustaE(String nombre,
			Cliente clienteActual) throws NombreInvalidoException {
			...
			if (c instanceof EntidadNoEstatal) {
				if (c.getNombre().equalsIgnoreCase(nombre)
					...
					throw new NombreInvalidoException(
							"Esa Entidad Estatal ya existe en nuestro sistema");
				}
			}
		}
	}

	private void validarNombreEdicionRobustaJ(String nombre,
			Cliente clienteActual) throws NombreInvalidoException {
		...
		for (Cliente c : EmpresaTelecomunicaciones.getInstancia().getClientes()) {
			if (c instanceof PersonaJuridica) {
				if (c.getNombre().equalsIgnoreCase(nombre)
						&& c != clienteActual) {
					throw new NombreInvalidoException(
							"Esa Persona Juridica ya existe en nuestro sistema");
				}
			}
		}
	}

	private void validarDireccionEdicion(String direccion)
			throws UbicacionInvalidaException {

		Cliente.validarDireccion(direccion);
	}

	private void validarNumIdEdicion(String numId, Cliente cliente)
			throws CarnetIdentidadInvalidoException {

		PersonaNatural.validarNumId(numId);

		for (Cliente c : EmpresaTelecomunicaciones.getInstancia().getClientes()) {
			if (c instanceof PersonaNatural) {
				if (((PersonaNatural) c).getNumId().equals(numId)
						&& c != cliente) {
					throw new CarnetIdentidadInvalidoException(
							"Ya existe una persona con este número de identidad");
	...
	private void validarOrganismoEdicion(String organismo)
			throws NombreInvalidoException {
	...
	private void configurarMenuContextual() {
		...
		menuEditar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selectedRow >= 0) {
					nombreClienteSeleccionado = (String) tableModel.getValueAt(
							selectedRow, 0);
					clienteSeleccionado = EmpresaTelecomunicaciones
							.getInstancia().buscarCliente(
		...
		menuEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (selectedRow >= 0) {
					...
					String nombreCliente = (String) tableModel.getValueAt(
							selectedRow, 0);
					...
					if (confirm == JOptionPane.YES_OPTION) {
						boolean eliminado = EmpresaTelecomunicaciones
								.getInstancia().eliminarCliente(nombreCliente);

						if (eliminado) {
							if (nombreClienteSeleccionado != null
									&& nombreClienteSeleccionado
											.equals(nombreCliente)) {
								cerrarPanelEdicion();
							}
	...
	// Crear una PERSONA NATURAL validando todos sus datos y lanzando
	...
	public boolean crearPersonaNatural(Servicio servicioSeleccionado) {
		...
		try {
			...
			for (Representante r : EmpresaTelecomunicaciones.getInstancia()
					.getRepresentantes()) {
		...
		if (errores.length() > 0) {
			...
			if (servicioSeleccionado != null) {
				try {
					...
					EmpresaTelecomunicaciones.getInstancia()
							.agregarPersonaNatural(nombre, direccion,
									municipio, provincia, numId);
					creado = true;
	...
	public boolean crearPersonaJuridica(Servicio servicioSeleccionado,
			...
			for (Cliente c : EmpresaTelecomunicaciones.getInstancia()
				...
				if (c instanceof PersonaJuridica) {
					if (c.getNombre().equals(txtNombreCreate.getText())) {
						throw new Exception(
								"Esa Persona Jurídica ya existe en nuestro sistema");
					}
				}
		...
		if (errores.length() > 0) {
			...
			if (servicioSeleccionado != null
				...
				try {
					...
					// Usar los valores ya valido
					EmpresaTelecomunicaciones.getInstancia()
							.agregarPersonaJuridica(nombre, direccion,
	...
	// Metodo para crear una PERSONA JURIDICA validando todos sus datos
	public boolean crearEntidadNoEstatal(Servicio servicioSeleccionado,
			Representante representanteSeleccionado) {
		...
		if (errores.length() > 0) {
			...
			if (servicioSeleccionado != null
				...
				try {
					...
					// Usar los valores ya valido
					EmpresaTelecomunicaciones.getInstancia()
							.agregarEntidadNoEstatal(nombre, direccion,
									representanteSeleccionado);
					creado = true;
					// Limpiar campos después de creación exitosa
					limpiarCampos();
	...
	private void manejarError(Exception e, String mensaje) {
		UIManager.put("OptionPane.messageFont", new Font("Serif", Font.PLAIN,
				18));
